...
/**
  ...
  */
static uint16_t __ee_find_valid_page(uint8_t Operation) {
    /* */
...
/**
  ...
  */
static uint16_t __ee_verify_page_full_write_varible(uint16_t VirtAddress, uint16_t Data) {
  HAL_StatusTypeDef FlashStatus = HAL_OK;
  uint16_t ValidPage = PAGE0;
  uint32_t Address = 0x0803E000, PageEndAddress = 0x0803E000;

  /* Get valid Page for write operation */
  ValidPage = __ee_find_valid_page(WRITE_IN_VALID_PAGE);
...
/**
  ...
  *           - Flash error code: on write Flash error
  */
static uint16_t __ee_page_transfer(uint16_t VirtAddress, uint16_t Data) {
  HAL_StatusTypeDef FlashStatus = HAL_OK;
  uint32_t NewPageAddress = 0x080313FF, OldPageAddress = 0x08031000;
  uint16_t ValidPage = PAGE0, VarIdx = 0;
  uint16_t EepromStatus = 0, ReadStatus = 0;

  /* Get active Page for read operation */
  ValidPage = __ee_find_valid_page(READ_FROM_VALID_PAGE);
  ...
  EepromStatus = __ee_verify_page_full_write_varible(VirtAddress, Data);
  /* If program operation was failed, a Flash error code is returned */
  ...
  for (VarIdx = 0; VarIdx < NumbOfVar; VarIdx++)
  {
    if (VarIdx != VirtAddress)  /* Check each variable except the one passed as parameter */
    {
      ...
      if (ReadStatus != 0x1)
      {
        ...
        EepromStatus = __ee_verify_page_full_write_varible(VarIdx, DataVar);
        /* If program operation was failed, a Flash error code is returned */
        if (EepromStatus != HAL_OK)
        {
          return EepromStatus;
...
uint16_t EE_ReadVariable(uint16_t VirtAddress, uint16_t* Data) {
  ...
  uint32_t Address = 0x0803E000, PageStartAddress = 0x0803E000;
...
uint16_t EE_WriteVariable(uint16_t VirtAddress, uint16_t Data) {
  ...
  Status = __ee_verify_page_full_write_varible(VirtAddress, Data);

  /* In case the EEPROM active page is full */
  if (Status == PAGE_FULL)
  {
    /* Perform Page transfer */
    Status = __ee_page_transfer(VirtAddress, Data);
  }
